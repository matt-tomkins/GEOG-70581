...
Using this vector of column names, we are going to create a new data frame (called `training_variables`) containing only the independent variables of interest for the `training` dataset:
...
To run a linear regression using these data, we need to ensure our dependent variable of interest is present in the dataframe, which can be achieved using `cbind`.

```{r, echo = TRUE, eval = TRUE, warning= FALSE, message = FALSE, fig.align = 'center'}
# Column bind (cbind) the dependent (n=1) and independent variables (n=16)
model_df <- cbind(variable = training$Zn, training_df)
```

> Run the above code which creates a new dataframe (`model_df`) containing the dependent variable we want to model (`Zn`) and the full set of independent variables for our training set. In this case, we are using the generic term `variable` to denote our dependent variable.
...
> Next, run a new `lm()` model using all the independent variables, which can be acheived using the equation `Zn ~ .`, and inspect the output.

> How has model performance changed?

If all has worked as intended, you should have noted improved model performance on the training dataset (e.g., larger R^2^ value). This seems great!

#### Problems with the full models

*However*, there are some important problems with this approach:

-   While adding all the independent variables to the model is quite simple, we don't know which are actually **important**. Perhaps only one or two of these have a demonstrable and/or high magnitude effect on the dependent variable?
...
-   Another, and perhaps more important issue, is that we not yet assessed the performance of the model on the **testing** data set. Good performance on the training set is a good sign, but we need to evaluate how the model performs on unseen and independent data.

To achieve this, we can use the `predict()` function which predicts new values for `Zn` based on our model, and can be achieved using the following code:

```{r, echo = TRUE, eval = TRUE, warning= FALSE, message = FALSE, fig.align = 'center'}
# Predict Zn values based upon linear model, saving to testing dataframe
testing$predicted_Zn <- predict(linear_model, newdata = testing)
```

> Run the above code, which will create a new column in the `testing` dataframe called `predicted_Zn`, based on the model (`linear_model`) and the catchment characteristics stored in `testing`:

> To evaluate the model perform, make a plot of measured `Zn` *vs.* predicted `Zn` values, using the skills developed in the [Introduction to R](@Intro_to_R). I would recommend adding a 1:1 line to the plot, as follows: `geom_abline(intercept = 0, slope = 1, lty = "dashed")`. If you want, you could also add a simple linear regression, which can be acheived using the following: `geom_smooth(method = "lm", se = FALSE, colour="#FF953C")`.
...
While this is useful to allow for visual assessment of performance, we might also want to produce some metrics for a quantitative assessment, such as root-mean-square error (RMSE), normalised root-mean-square-error (nRMSE), or R^2^. 

It is *tempting* at this stage to produce a regression between the measured and modelled values, such as the following: 
...
While this looks like a sensible approach, and it gives a high R^2^ value in this case, this is an **inappropriate** approach and is an easy mistake, because this is evaluating the deviations from the modelled line-of-best-fit (shown in orange [#FF953C] above), and not the 1:1 line. 

**Instead**, we need to calculate the model R^2^ manually, the equation for which is here:

$$R^2=1-\frac{RSS}{TSS}$$
Here $RSS$ is the sum of the squares of residuals i.e., deviations of predicted values (`predicted_Zn`) from actual empirical values (`Zn`), and can be calculated in R as follows:

```{r, echo = TRUE, eval = TRUE, warning= FALSE, message = FALSE, fig.align = 'center'}
# Sum of Squared Residuals: sum of squared differences between measured and modelled value
rss <- sum((testing$Zn - testing$predicted_Zn)^2)
```

$TSS$ refers to the total sum of squares i.e., the dispersion of the data points from the average value.

To calculate $TSS$, you need to calculate the difference between each measured value and the mean of the measured values (e.g., `mean`), square each value, and then calculate the total sum. 


```{r, echo = FALSE, eval = TRUE, warning= FALSE, message = FALSE, fig.align = 'center'}
# Sum of Squares Total:
tss <- sum((testing$Zn - mean(testing$Zn))^2)
```

> Try to calculate $TSS$ using the guidance above, and using the R code for $RSS$ as a guide. If you get stuck, ask for help! 

The ratio of $RSS$ to $TSS$ accounts for the *unexplained* variation in the dependent variable that is not explained by the model. In turn, using $$1-\frac{RSS}{TSS}$$ returns our *explained* variation, which produces:
...
As you can see, we have an unexpected result here, a strongly negative R^2^ ($<0$). How can we account for this? 

Numerically, this has occurred because $RSS>TSS$ i.e., in general the residuals (deviation between modelled and measured values) are larger than the difference between the measured values and their average. But what does this actually **mean**?

The explanation is that our model predictions are worse than if we simply used the average of the measured values for our prediction. 

What does this suggest? Our model is **overfit** i.e., it performs well on the training dataset, but performs very poorly on the testing dataset. 
...
-   No need to pick, just use elastic net regression, which is a combination of both = sum of squared residuals + lasso regression penalty + ridge regression penalty
